...
# --- ۳. رابط کاربری (UI) ---
st.set_page_config(page_title="Aariz Precision Station V6.2", layout="wide")
...
if uploaded_file and len(models) == 3:
    ...
    with col2:
        ...
        if all(k in l for k in [10, 4, 0, 2, 18, 22, 17, 21, 15, 5, 14, 3, 20, 21, 23, 17, 8, 27, 12, 13]):
            # خطوط مرجع Steiner & Downs (بدون تغییر)
    ...
    # --- ۴. محاسبات تجمعی (تضمین عدم حذف + افزایشی) ---
    st.divider()
    ...
    # McNamara Calculations (افزایشی)
    co_a = get_dist(l[12], l[0]) # Effective Maxilla
    co_gn = get_dist(l[12], l[13]) # Effective Mandible
    diff = round(co_gn - co_a, 2)
    
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("McNamara Diff", f"{diff} mm", f"Co-A: {co_a}")
    m2.metric("Effective Mandible", f"{co_gn} mm")
    m3.metric("Steiner (ANB)", f"{anb}°")
    m4.metric("Growth (FMA)", f"{fma}°")

    # --- ۵. گزارش نهایی و نقشه راه درمان (Gender-Aware) ---
    st.divider()
    st.header(f"📑 گزارش جامع تحلیلی ({gender})")
    rep_c1, rep_c2 = st.columns(2)
    with rep_c1:
        st.subheader("📐 آنالیز McNamara (طول فکین)")
        # رفرنس‌های مک‌نامارا بر اساس جنسیت و طول فک بالا
        st.write(f"• طول موثر فک بالا (Co-A): **{co_a} mm**")
        st.write(f"• طول موثر فک پایین (Co-Gn): **{co_gn} mm**")
        st.write(f"• اختلاف طول فکین (Diff): **{diff} mm**")
        
        # تفسیر مک‌نامارا
        if gender == "خانم (Female)":
            ideal_diff = 22 if co_a < 90 else 27
        else:
            ideal_diff = 27 if co_a < 95 else 32
            
        if diff > ideal_diff + 3: st.warning("⚠️ مندیبل پروگناتیک (بزرگ‌تر از نرمال نسبت به ماکسیلا)")
        elif diff < ideal_diff - 3: st.warning("⚠️ مندیبل رتروگناتیک (کوچک‌تر از نرمال)")

    with rep_c2:
        st.subheader("💡 نقشه راه درمان و بافت نرم")
        # حفظ تحلیل بافت نرم قبلی
        ...
        st.write(f"• فاصله لب پایین تا E-Line: **{dist_li} mm**")
        
        if abs(diff - ideal_diff) > 10:
            st.error("🚨 ناهنجاری شدید فکی بر اساس طول موثر؛ اولویت جراحی فک (Surgical) بالا است.")
        else:
            st.success("✅ ناهنجاری در محدوده جبران ارتودنسی (Camouflage).")
