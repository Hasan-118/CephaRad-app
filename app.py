...
def get_model_map():
    return [
        'checkpoint_unet_clinical_int8.pth',
        'specialist_pure_model_int8.pth',
        'tmj_specialist_model_int8.pth'
    ]
...
def generate_clinical_pdf(patient_info, norm_table_data, detailed_interpretations, treatment_plan, annotated_img_bytes):
    ...
    title_style = ParagraphStyle(
        'DocTitle', parent=styles['Heading1'], fontName=VAZIR_FONT_NAME,
        fontSize=16, leading=20, textColor=colors.HexColor('#1E3A8A'),
        alignment=1, spaceAfter=8
    )
    section_style = ParagraphStyle(
        'SectionHeader', parent=styles['Heading2'], fontName=VAZIR_FONT_NAME,
        fontSize=12, leading=15, textColor=colors.HexColor('#1E40AF'),
        spaceBefore=8, spaceAfter=4, alignment=2
    )
    normal_style = ParagraphStyle(
        'DocNormal', parent=styles['Normal'], fontName=VAZIR_FONT_NAME,
        fontSize=10, leading=14, alignment=2
    )
    elements = []
    elements.append(Paragraph(
        reshape_fa("Aariz Precision Station - Comprehensive Cephalometric Report"),
        title_style
    ))
    ...
    elements.append(Paragraph(
        reshape_fa("۱. مقایسه با پارامترهای مرجع و استاندارد (Norms)"),
        section_style
    ))
    table_content = [[
    ...
    if annotated_img_bytes:
        elements.append(Paragraph(
            reshape_fa("۴. تصویر سفلومتری و لندمارک‌های رسم‌شده"),
            section_style
        ))
...
# ============================================================
# ۱۲. تفکیک رابط کاربری به تب‌های اصلی
# ============================================================
tab_ceph, tab_3d = st.tabs([
    "📐 آنالیز سئفالومتری (۲D)",
    "🦷 آنالیز اسکن داخل دهانی (۳D)"
])
...
with tab_ceph:
    ...
    pixel_size = st.sidebar.number_input(
        "Pixel Size (mm/px):", 0.01, 1.0, 0.1, 0.001, format="%.4f"
    )
    text_scale = st.sidebar.slider("🔤 مقیاس نام لندمارک:", 1, 10, 2)
    uploaded_file = st.sidebar.file_uploader(
        "آپلود تصویر:", type=['png', 'jpg', 'jpeg']
    )
    ...
    def run_precise_prediction(img_pil, models):
        ...
        with torch.no_grad():
            ...
            coords = {}
            for i in range(29):
                hm = outs[1][i] if i in ANT_IDX else (
                    outs[2][i] if i in POST_IDX else outs[0][i]
                )
    ...
    if uploaded_file:
        if "raw_img" not in st.session_state or \
           st.session_state.get("file_id") != uploaded_file.name:
            st.session_state.raw_img = Image.open(uploaded_file).convert("RGB")
            st.session_state.file_id = uploaded_file.name
            with st.spinner("🧠 در حال تحلیل با مدل‌های بهینه‌شده..."):
                st.session_state.initial_lms = run_precise_prediction(
                    st.session_state.raw_img, models
                )
        ...
        target_idx = st.sidebar.selectbox(
            "🎯 انتخاب لندمارک:", range(29),
            format_func=lambda x: f"{x}: {landmark_names[x]}"
        )

        if st.sidebar.button("🔄 Reset Point"):
            st.session_state.lms[target_idx] = \
                st.session_state.initial_lms[target_idx].copy()
        ...
        with col1:
            ...
            mag_crop = raw_img.crop(
                (left, top, left + size_m, top + size_m)
            ).resize((300, 300), Image.LANCZOS)
            mag_draw = ImageDraw.Draw(mag_crop)
            mag_draw.line((135, 150, 165, 150), fill="red", width=2)
            mag_draw.line((150, 135, 150, 165), fill="red", width=2)
            res_mag = streamlit_image_coordinates(
                mag_crop, key=f"mag_{target_idx}_{st.session_state.click_version}"
            )
            if res_mag:
                scale_mag = size_m / 300
                new_c = [
                    int(left + (res_mag["x"] * scale_mag)),
                    int(top + (res_mag["y"] * scale_mag))
                ]
        ...
        with col2:
            ...
            def sc(p):
                return (int(p[0] * ratio_disp), int(p[1] * ratio_disp))

            if all(k in l for k in [10, 4, 0, 2, 18, 22, 17, 21,
                                    15, 5, 14, 3, 20, 21, 23, 17, 8, 27]):
            ...
            for i, pos in l.items():
                ...
                draw.ellipse(
                    [s_pos[0] - r, s_pos[1] - r, s_pos[0] + r, s_pos[1] + r],
                    fill=color, outline="white"
                )
                if text_scale > 1:
                    draw.text(
                        (s_pos[0] + 8, s_pos[1] - 4),
                        landmark_names[i], fill=color
                    )

            res_main = streamlit_image_coordinates(
                draw_img, key=f"main_{st.session_state.click_version}"
            )
            if res_main:
                m_c = [
                    int(res_main["x"] / ratio_disp),
                    int(res_main["y"] / ratio_disp)
                ]
        ...
        def get_ang(p1, p2, p3, p4=None):
            ...
            return round(np.degrees(np.arccos(
                np.clip(np.dot(v1, v2) / (n if n > 0 else 1), -1, 1)
            )), 2)
        ...
        wits_mm = round((
            np.dot(np.array(l[0]) - p_occ_p, v_occ)
            - np.dot(np.array(l[2]) - p_occ_p, v_occ)
        ) * pixel_size, 2)
        wits_norm = 0.0 if gender == "آقا (Male)" else -1.0

        dist_ls = round(
            dist_to_line(np.array(l[25]), np.array(l[8]), np.array(l[27])) * pixel_size, 2
        )
        dist_li = round(
            dist_to_line(np.array(l[24]), np.array(l[8]), np.array(l[27])) * pixel_size, 2
        )

        norm_table_data = [
            {"param": "SNA (Maxilla Pos)", "measured": sna, "unit": "°", "norm": 82.0,
             "dev": round(sna - 82.0, 2),
             "status": "Protrusive" if sna > 84 else ("Retrusive" if sna < 80 else "Normal")},
            {"param": "SNB (Mandible Pos)", "measured": snb, "unit": "°", "norm": 80.0,
             "dev": round(snb - 80.0, 2),
             "status": "Protrusive" if snb > 82 else ("Retrusive" if snb < 78 else "Normal")},
            {"param": "ANB (Skeletal Rel)", "measured": anb, "unit": "°", "norm": 2.0,
             "dev": round(anb - 2.0, 2),
             "status": "Class II" if anb > 4 else ("Class III" if anb < 0 else "Class I")},
            {"param": "Wits Appraisal", "measured": wits_mm, "unit": "mm",
             "norm": wits_norm, "dev": round(wits_mm - wits_norm, 2),
             "status": "Class II" if (wits_mm - wits_norm) > 1.5 else (
                 "Class III" if (wits_mm - wits_norm) < -1.5 else "Class I")},
            {"param": "FMA (Vertical Angle)", "measured": fma, "unit": "°", "norm": 25.0,
             "dev": round(fma - 25.0, 2),
             "status": "Hyperdivergent" if fma > 30 else ("Hypodivergent" if fma < 20 else "Normal")},
            {"param": "Co-A (Maxilla Length)", "measured": co_a, "unit": "mm", "norm": 90.0,
             "dev": round(co_a - 90.0, 2),
             "status": "Increased" if co_a > 95 else ("Decreased" if co_a < 85 else "Normal")},
            {"param": "Co-Gn (Mandible Length)", "measured": co_gn, "unit": "mm", "norm": 115.0,
             "dev": round(co_gn - 115.0, 2),
             "status": "Increased" if co_gn > 122 else ("Decreased" if co_gn < 108 else "Normal")},
            {"param": "Upper Lip to E-Line", "measured": dist_ls, "unit": "mm", "norm": -4.0,
             "dev": round(dist_ls - (-4.0), 2),
             "status": "Protrusive" if dist_ls > -2 else ("Retrusive" if dist_ls < -6 else "Normal")},
            {"param": "Lower Lip to E-Line", "measured": dist_li, "unit": "mm", "norm": -2.0,
             "dev": round(dist_li - (-2.0), 2),
             "status": "Protrusive" if dist_li > 0 else ("Retrusive" if dist_li < -4 else "Normal")}
        ]

        detailed_interpretations = {
            "رابطه اسکلتی ساژیتال (Sagittal Relationship)":
                f"مقدار زاویه ANB برابر با {anb}° و ارزیابی Wits برابر با {wits_mm} mm می‌باشد. "
                + ("نشان‌دهنده ناهنجاری اسکلتی کلاس II شدید به دلیل برآمدگی فک بالا یا عقب‌ماندگی فک پایین است."
                   if anb > 4.5 else
                   ("نشان‌دهنده ناهنجاری اسکلتی کلاس III به دلیل جلو بودن فک پایین یا ضعیف بودن فک بالا است."
                    if anb < 0.5 else
                    "رابطه فک بالا و پایین در حد فاصل نرمال است و تطابق اسکلتی کلاس I وجود دارد.")),
            "الگوی رشد عمودی (Vertical Pattern)":
                f"زاویه FMA برابر با {fma}° است (Norm: 25.0°). "
                + ("الگوی رشد هایپردایورجنت (Vertical Growing / High Angle)."
                   if fma > 30 else
                   ("الگوی رشد هایپودایورجنت (Horizontal Growing / Low Angle)."
                    if fma < 20 else
                    "الگوی رشد عمودی متوازن و نرمال (Mesofacial/Normodivergent).")),
            "تحلیل تناسب طول فکین (McNamara Discrepancy)":
                f"طول موثر فک بالا (Co-A) برابر {co_a} mm و طول موثر فک پایین "
                f"(Co-Gn) برابر {co_gn} mm است (اختلاف: {diff_mcnamara} mm). "
                + ("اختلاف طول فکین بیشتر از حد نرمال."
                   if diff_mcnamara > 28 else
                   ("اختلاف طول فکین کمتر از حد نرمال."
                    if diff_mcnamara < 20 else
                    "تناسب طولی فک بالا و پایین در محدوده هارمونیک قرار دارد.")),
            "پروفایل بافت نرم (Soft Tissue Profile)":
                f"فاصله لب بالا تا خط E برابر با {dist_ls} mm و لب پایین {dist_li} mm است. "
                + ("برجستگی بافت نرم لب‌ها نسبت به خط E مشهود است."
                   if dist_ls > -1 or dist_li > 1 else
                   ("پروفایل لب‌ها نسبت به خط E عقب‌رفته (Retrusive) است."
                    if dist_ls < -6 else
        ...
        if anb > 4:
            if fma > 30:
                treatment_plan.append({
                    "title": "کنترل رشد عمودی و اصلاح کلاس II",
                    "desc": "استفاده از دستگاه‌های اینترودکننده یا Tilted Occlusal Plane control همراه با الستیک‌های کلاس II."
                })
            else:
                treatment_plan.append({
                    "title": "تحریک رشد/پیش‌آوردن فک پایین",
                    "desc": "دستگاه‌های فانکشنال (مانند Twin Block یا Herbst) در سنین رشد، یا جراحی BSSO در سنین بالاتر."
                })
        elif anb < 0:
            treatment_plan.append({
                "title": "پروتراکشن فک بالا یا اصلاح کلاس III",
                "desc": "فیس‌ماسک در سنین رشد یا جراحی دو فک در سنین بالاتر."
            })
        else:
            treatment_plan.append({
                "title": "ارتودنسی مرتب‌سازی دندانی",
                "desc": "تمرکز بر ردیف‌سازی دندان‌ها، اصلاح Crowding و تنظیم قوس‌ها بدون مداخله اسکلتی."
            })

        if dist_ls > 0 or dist_li > 1:
            treatment_plan.append({
                "title": "ارزیابی طرح درمان کشیدن دندان (Extraction Evaluation)",
                "desc": "به دلیل برجستگی لب‌ها نسبت به خط E، بررسی کشیدن پری‌مولرها جهت ریترود کردن دندان‌های قدامی پیشنهاد می‌شود."
            })
        ...
        patient_info_pdf = {
            ...
            'date': '2026-09-10',
            'diag': "Class II Skeletal" if anb > 4 else (
                "Class III Skeletal" if anb < 0 else "Class I Skeletal"
            )
        }
        ...
        pdf_bytes = generate_clinical_pdf(
            patient_info_pdf, norm_table_data,
            detailed_interpretations, treatment_plan,
            annotated_img_bytes
        )
...
with tab_3d:
    if render_intraoral_3d_tab is not None:
        ...
        st.warning(
            "⚠️ ماژول `intraoral_3d_module.py` در کنار فایل اصلی یافت نشد. "
            "لطفاً این فایل را در مسیر برنامه قرار دهید."
        )
