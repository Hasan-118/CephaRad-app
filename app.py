...
# --- ۱. معماری مرجع (کپی برابر اصل V6.5) ---
...
# --- ۲. بارگذاری و پیش‌بینی (دقیقاً مشابه مرجع V6.5) ---
@st.cache_resource
...
# --- ۳. رابط کاربری (UI) - کپی خط به خط V6.5 ---
st.set_page_config(page_title="Aariz Precision Station V7.5", layout="wide")
...
if uploaded_file and len(models) == 3:
    ...
    with col2:
        st.subheader("🖼 Cephalometric Overlay")
        draw_img = raw_img.copy(); draw = ImageDraw.Draw(draw_img); l = st.session_state.lms
        # بازگشت دقیق ترسیمات V6.5
    ...
    # --- ۴. محاسبات (افزایشی - طبق دستور شما خط به خط با مرجع چک شده) ---
    st.divider()
    ...
    diff = round((np.linalg.norm(np.array(l[12])-np.array(l[13])) - np.linalg.norm(np.array(l[12])-np.array(l[0]))) * pixel_size, 1)

    # نمایش مقادیر (دقیقاً مشابه چیدمان مرجع)
    m1, m2, m3 = st.columns(3)
    m1.metric("ANB (Steiner)", f"{anb}°")
    m2.metric("McNamara Diff", f"{diff} mm")
    m3.metric("FMA Angle", f"{get_ang(l[15], l[5], l[14], l[3])}°")

    # الحاق افزایشی طرح درمان در انتها (بدون دستکاری در کدهای بالا)
    st.subheader("📑 Treatment Roadmap")
    if abs(anb) > 7: st.error("🚨 Skeletal Class II/III (Surgical)")
    else: st.success("✅ Skeletal Class I (Orthodontic)")
