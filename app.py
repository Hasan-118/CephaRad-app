...
# استفاده از کتابخانه استاندارد fpdf
...
if uploaded_file and len(models) == 3:
    ...
    # --- ۵. بخش دانلود PDF ایمن (بدون نیاز به کتابخانه خارجی سنگین) ---
    if st.button("📥 دریافت گزارش PDF"):
        ...
        # استفاده از متن انگلیسی خالص برای جلوگیری از تداخل انکودینگ سرور
        pdf.cell(200, 10, txt=f"Analysis for: {gender}", ln=True)
        pdf.cell(200, 10, txt=f"Steiner ANB: {anb} deg", ln=True)
        pdf.cell(200, 10, txt=f"McNamara Diff: {diff_mcnamara} mm", ln=True)
        pdf.cell(200, 10, txt=f"FMA: {fma} deg", ln=True)
        
        # تولید امن بایت‌ها
        pdf_bytes = pdf.output(dest='S').encode('latin-1', 'ignore')
        b64 = base64.b64encode(pdf_bytes).decode()
        href = f'<a href="data:application/pdf;base64,{b64}" download="Aariz_Report.pdf" style="text-decoration:none; background-color:#4CAF50; color:white; padding:10px 20px; border-radius:5px;">Download Report PDF</a>'
