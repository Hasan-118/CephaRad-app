...
# --- ۱. معماری کامل DoubleConv و CephaUNet (۱۰۰٪ مطابق مرجع V7.8) ---
...
# --- ۲. لودر مدل‌ها و پیش‌بینی هوشمند (فراخوانی هر ۳ مدل) ---
@st.cache_resource
...
def run_precise_prediction(img_pil, models, device):
    ...
    coords = {}
    for i in range(29):
        # منطق تفکیک نواحی تخصصی مدل‌ها
        hm = outs[1][i] if i in ANT_IDX else (outs[2][i] if i in POST_IDX else outs[0][i])
        y, x = np.unravel_index(np.argmax(hm), hm.shape)
        coords[i] = [int((x - px) / ratio), int((y - py) / ratio)]
    return coords

# --- ۳. رابط کاربری Streamlit و منطق مدیریت تصویر ---
...
if uploaded_file and len(models) == 3:
    ...
    # --- ۴. محاسبات و تفسیر بالینی (دقیقاً مطابق Gold Standard) ---
    st.divider()
