...
# --- ۳. رابط کاربری (UI) ---
st.set_page_config(page_title="Aariz Precision Station V6.9", layout="wide")
...
if uploaded_file and len(models) == 3:
    ...
    with col2:
        ...
        if all(k in l for k in [10, 4, 0, 2, 18, 22, 17, 21, 15, 5, 14, 3, 20, 21, 23, 17, 8, 27, 12, 13]):
            ...
            draw.line([tuple(l[12]), tuple(l[0])], fill="brown", width=2)
            draw.line([tuple(l[12]), tuple(l[13])], fill="brown", width=2)
    ...
    # --- ۴. محاسبات و طرح درمان (بازگشت به Gold Standard) ---
    st.divider()
    ...
    m1.metric("ANB (Steiner)", f"{anb}°")
    m2.metric("Wits (Occ Plane)", f"{round(wits, 1)} mm")
    m3.metric("McNamara Diff", f"{diff} mm")

    # --- بخش طرح درمان (Treatment Roadmap) ---
    st.divider()
    st.header("📑 تشخیص و نقشه راه درمان (Clinical Roadmap)")
    c1, c2 = st.columns(2)
    with c1:
        st.subheader("💡 تشخیص اسکلتال")
        wits_norm = 0 if gender == "آقا (Male)" else -1
        class_status = "II" if wits > wits_norm + 2 else "III" if wits < wits_norm - 2 else "I"
        st.write(f"• فک بیمار در وضعیت **Skeletal Class {class_status}** قرار دارد.")
        
        if abs(anb) > 7 or abs(diff - 25) > 12:
            st.error("🚨 ناهنجاری شدید: احتمال نیاز به **Orthognathic Surgery** بالا است.")
        else:
            st.success("✅ ناهنجاری متوسط: قابل درمان با متدهای **Camouflage** یا **Growth Modification**.")

    with c2:
        st.subheader("👄 تحلیل زیبایی و بافت نرم")
        ...
        st.write(f"• لب بالا تا خط E: **{dist_ls} mm** {'(Protrusive)' if dist_ls > -2 else '(Retrusive)' if dist_ls < -6 else '(Normal)'}")
        st.write(f"• لب پایین تا خط E: **{dist_li} mm** {'(Protrusive)' if dist_li > 0 else '(Retrusive)' if dist_li < -4 else '(Normal)'}")
